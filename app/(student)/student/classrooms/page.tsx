  ...
  return (
    <div className="min-h-[calc(100vh-80px)] bg-background relative overflow-hidden px-4 sm:px-6 md:px-10 pt-4 pb-12">
      ...
      <div className="absolute inset-0 w-full h-full pointer-events-none overflow-hidden -z-10 transform-gpu">
        <div className="w-full h-full bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-[#5D7B6F]/15 via-emerald-500/10 to-transparent blur-3xl opacity-50 transform-gpu" />
      </div>
      ...
      <div className="max-w-6xl mx-auto space-y-6 relative z-10">
        ...
        <motion.div
          ...
          className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white/80 backdrop-blur-xl p-6 sm:p-7 rounded-3xl border border-slate-200/80 shadow-xs"
        >
          <div className="space-y-1.5">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#5D7B6F]/10 text-[#5D7B6F] text-[11px] font-black uppercase tracking-wider border border-[#5D7B6F]/20">
              <Sparkles className="w-3.5 h-3.5" /> Lớp học & Giảng dạy
            </div>
            <h1 className="text-2xl sm:text-3xl font-black tracking-tight text-slate-900 flex items-center gap-3">
              <GraduationCap className="w-7 h-7 text-[#5D7B6F]" />
          ...
          <Button 
            ...
            className="bg-[#5D7B6F] hover:bg-[#4A6359] text-white shadow-md shadow-[#5D7B6F]/20 font-black text-xs uppercase tracking-wider gap-2 rounded-2xl h-12 px-6 transition-all cursor-pointer shrink-0 active:scale-[0.98]"
          >
        ...
        {successMessage && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            className="p-4 bg-emerald-50 text-emerald-800 rounded-2xl border border-emerald-200 flex items-center gap-3 shadow-2xs text-xs font-bold"
          >
        ...
        {loading ? (
          <div className="py-20 text-center text-slate-500 font-semibold">
            <Loader2 className="w-7 h-7 animate-spin mx-auto text-[#5D7B6F]" />
            <p className="text-xs mt-2 font-bold text-slate-400">Đang tải thông tin lớp học...</p>
          </div>
        ) : classrooms.length === 0 ? (
          <motion.div
            initial={{ opacity: 0, scale: 0.98 }}
            animate={{ opacity: 1, scale: 1 }}
            className="py-16 text-center border-2 border-dashed border-slate-300 rounded-3xl bg-white/70 backdrop-blur-md p-8 space-y-4 shadow-xs"
          >
            <div className="w-14 h-14 rounded-2xl bg-[#5D7B6F]/10 text-[#5D7B6F] flex items-center justify-center mx-auto">
              <Users className="w-7 h-7" />
            </div>
            <div className="space-y-1">
              <h3 className="text-lg font-black text-slate-900">Bạn chưa tham gia lớp học nào</h3>
            ...
            <Button
              onClick={() => setJoinModalOpen(true)}
              className="bg-[#5D7B6F] hover:bg-[#4A6359] text-white font-black text-xs uppercase tracking-wider rounded-2xl px-6 h-11 cursor-pointer shadow-sm"
            >
            ...
            <div className="space-y-3.5">
              <div className="flex items-center justify-between px-1">
                <h2 className="text-sm font-black uppercase tracking-wider text-slate-700 flex items-center gap-2">
                  <Users className="w-4 h-4 text-[#5D7B6F]" /> Lớp đã tham gia ({classrooms.length})
                </h2>
              </div>
                  ...
                  return (
                    <Card
                      ...
                      className={`cursor-pointer transition-all duration-300 border rounded-2xl overflow-hidden relative group p-4 ${
                        isSelected
                          ? 'border-[#5D7B6F] ring-2 ring-[#5D7B6F]/20 bg-[#5D7B6F]/5 shadow-sm'
                          : 'border-slate-200/80 bg-white/80 backdrop-blur-md hover:border-[#5D7B6F]/50 hover:shadow-xs'
                      }`}
                    >
                      <div className="flex items-start justify-between gap-2">
                        <div className="flex items-start gap-3 min-w-0">
                          <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 font-black text-xs ${
                            isSelected ? 'bg-[#5D7B6F] text-white' : 'bg-slate-100 text-slate-600 group-hover:bg-[#5D7B6F]/10 group-hover:text-[#5D7B6F]'
                          }`}>
