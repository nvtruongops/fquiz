...
export default async function QuizLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <AppLayout 
      ...
      fixedHeight={true}
    >
      <Suspense fallback={<div className="flex min-h-[60vh] items-center justify-center"><div className="h-10 w-10 animate-spin rounded-full border-2 border-primary border-t-transparent" /></div>}>
        {children}
      </Suspense>
    </AppLayout>
  )
