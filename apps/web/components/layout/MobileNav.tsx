          ...
          >
            ...
            <div className="space-y-1.5 pt-1 pb-24 sm:pb-28 overflow-y-auto custom-scrollbar flex-1 min-h-0">
              ...
              <Link
                ...
                onClick={(e) => {
                  ...
                  if (!user) {
                    e.preventDefault()
                    openAuthPrompt({
                      featureName: 'Bộ đề của tôi',
                      description: 'Lưu trữ, quản lý các bộ đề yêu thích và tự tạo đề thi cá nhân',
                      targetUrl: '/my-quizzes',
                    })
                  }
                }}
                className={cn(
                  "flex items-center justify-between px-3.5 py-3 rounded-2xl text-xs font-bold transition-all active:scale-98",
                  pathname === '/my-quizzes'
                    ? "bg-primary text-primary-foreground shadow-xs"
                    : "text-foreground hover:bg-muted"
                )}
              >
                ...
                {!user && <Lock className="w-3.5 h-3.5 text-muted-foreground shrink-0" />}
              </Link>
